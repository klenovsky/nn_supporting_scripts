...
class trans_probab():
    ...
    # Funkce pro nacitani dat z nextnano souboru 'filename',
    # stejne jako v strain_from_nextnano_to_csi.py
    def read_data_from_NN(self, filename):
        buffer = na.fromfile(filename,  sep='\n')
        return buffer

    # Funkce pro nacitani poctu datovych bodu, asi stejne
    # jako v strain_from_nextnano_to_csi.py, nadbytecna funkce
    ...
    # Funkce pro nacteni energie stavu s cislem 'state',
    # stejne jako v strain_from_nextnano_to_csi.py
    def retrieveWFEnergy(self, state, wftype='kp8'):
        f = open(os.path.join(self.prefix,
                              'wf_spectrum_dot_'+wftype+'.dat'), 'r')
        lines = f.readlines()
        return float(lines[state].split()[1])

    # Funkce pro nacteni rozmeru 8kp wf s poradovym cislem 'state',
    # stejne jako v strain_from_nextnano_to_csi.py
    def readSizesFromFld(self, state, wftype):
        fldname = os.path.join(self.prefix,
                               'wf_amplitude_real_dot_' + wftype
                               + '_1_0000_00%02i.fld' % (state))
        f = open(fldname, 'r')
        lines = [f.readline() for i in xrange(8)]
        ...
        # self.d = int(lines[7].split()[2])
        # NUMBER OF WINDOWS, NOT GRID_STEP
        # print 'Sizes %d, %d, %d readed from fld file (%d in total)' % \
        # (self.nx, self.ny, self.nz, self.n)

        # Funkce generujici Ep jako funkci prostorove souradnice,
        # pouzita je linearni interpolace mezi prislusnymi konstituenty

    def makeEp(self, coordNameToFit, state1=8,
               state2=9, wftype1='kp8', wftype2='kp8'):
        ...
        # Inline funkce, ktera provadi cele pretypovani,
        # mezi QD a SRL rozlisuje typickou limitou koncentrace
        # slitiny (<0.35 SRL, >0.35 QD)
        # PREFFERED FOR III-V
        EpTemp = [(x * 27.0 + (1.0 - x) * 28.8) if x > 0.35
                  else (x * 21.5 + (1.0 - x) * 28.8) for x in (data)]
        # InAs Ep = 21.5 eV
        # GaAs Ep = 28.8 eV
        # GaSb Ep = 27.0 eV
        # Si_(1-x)Ge_x QD attempt using method by Cardona
        # (Fundamentals of Semiconductors)
        # Eab = abs(self.retrieveWFEnergy(state1, wftype1)\
        # -self.retrieveWFEnergy(state2, wftype2))
        # print 'Eab ', Eab
        ...
        # so far phonon energy is subtracted,
        # so phonon is considered as emitted,
        # which should be the case for low temperatures
        # EpTemp = [(Eab - (x*EphGe+(1.0-x)*EphSi) - \
        # (x*EgGe+(1.0-x)*EgSi))**2.0*na.pi/8.0 for x in (data)]
        # Si_(1-x)Ge_x QD attempt using data from Richard
        # EpTemp = [(x*12.23+(1.0-x)*14.81) for x in (data)]
        # Si Ep = 14.81 eV   E_PX from Richard
        # PHYSICAL REVIEW B 70, 235204 (2004)
        # Ge Ep = 12.23 eV   E_PXd from Richard
        # PHYSICAL REVIEW B 70, 235204 (2004)
        # Si_(1-x)Ge_x QD attempt using method from Cardona
        # and data from Richard <del|p|gam> = sqrt(2*pi/hbar*10^
        # -4*(|<gam_15|p|gam_25'>|^2*|<gam_15|del_1>|^2)/(Egdel-Etrans)^2)
        # PREFFERED FOR SIGE
        ...
        # -self.retrieveWFEnergy(state2, wftype2))
        # print Eab
        # EpTemp = [ na.sqrt(2.0*na.pi/hbar*10**(-4.0)*
        # (x*EpxGe+(1.0-x)*EpxSi)**2.0*(x*0.7+(1.0-x)*0.75)**2.0
        # /(abs((x*EgGe+(1.0-x)*EgSi)-Eab))**2.0) for x in (data)]
        # Si Ep = 14.81 eV   E_PX from Richard
        # PHYSICAL REVIEW B 70, 235204 (2004)
        # Ge Ep = 12.23 eV   E_PXd from Richard
        # PHYSICAL REVIEW B 70, 235204 (2004)
        # Si_(1-x)Ge_x QD attempt using data from Richard <delta_1|p|gamma_25>
        # =  <gamma_15|p|gamma_25>*<gamma_15|delta_1>
        ...
        # EpTemp = [(x*EpxGe+(1.0-x)*EpxSi)*(x*0.7+(1.0-x)*0.75)
        # for x in (data)]
        # Si Ep = 14.81 eV   E_PX from Richard
        # PHYSICAL REVIEW B 70, 235204 (2004)
        # Ge Ep = 12.23 eV   E_PXd from Richard
        ...
        UncutCoords = na.asarray([coord[:nx], coord[nx:nx + ny],
                                  coord[nx + ny:nx + ny + nz]])
        UncutFitCoords = na.asarray([coordFit[:self.nx],
                                    coordFit[self.nx:self.nx + self.ny],
                                    coordFit[self.nx + self.ny:self.nx
                                             + self.ny + self.nz]])
        # Function for interpolating Ep to the grid of wavefunctions
        lo = na.array([UncutCoords[0][0],
                       UncutCoords[1][0], UncutCoords[2][0]])
        hi = na.array([UncutCoords[0][-1],
                       UncutCoords[1][-1], UncutCoords[2][-1]])
        # calling spline interpolation
        interfunc = Intergrid(EpUncut, lo=lo, hi=hi, order=0)
        # performing spline interpolation
        query_points = [([x, y, z]) for x in UncutFitCoords[0]
                        for y in UncutFitCoords[1] for z in UncutFitCoords[2]]
    ...
    def get_wf_names(self, state, wftype='kp8'):
        ...
        imag_suffixes = [self.IMAG_ROOT + wftype + '_' + str(i) +
                         '_0000_00%02i.dat' % (state) for i in xrange(1, 9)]
        real_names = [os.path.join(self.prefix, elem)
                      for elem in real_suffixes]
        imag_names = [os.path.join(self.prefix, elem)
                      for elem in imag_suffixes]
        return (real_names, imag_names)

    # Funkce generujici jmena 8kp wf souboru,
    # stejne jako v strain_from_nextnano_to_csi.py
    def generateWFNames(self, state, wftype='kp8'):
        if wftype == 'kp8':
            real = [os.path.join(self.prefix, 'wf_amplitude_real_dot_'
                                 + wftype + '_' + str(i)
                                 + '_0000_00%02i.dat' % (state))
                    for i in xrange(1, 9)]
            imag = [os.path.join(self.prefix, 'wf_amplitude_imag_dot_'
                                 + wftype + '_' + str(i)
                                 + '_0000_00%02i.dat' % (state))
                    for i in xrange(1, 9)]
        ...
        if wftype == 'kp6':
            ...
            self.allDatnames = real+imag
        if wftype == 'x1':
            real = [os.paht.join(self.prefix, 'wf_amplitude_real_dot_'
                                 + wftype + '_1_0000_00%02i.dat' % (state))]
            self.allDatnames = real
        if wftype == 'x2':
            real = [os.pah.join(self.prefix,
                                'wf_amplitude_real_dot_'
                                + wftype + '_1_0000_00%02i.dat' % (state))]
            self.allDatnames = real
        if wftype == 'x3':
            real = [os.path.join(self.prefix,
                                 'wf_amplitude_real_dot_' + wftype
                                 + '_1_0000_00%02i.dat' % (state))]
            self.allDatnames = real

    # Funkce vytvarejici 8kp wf z nextnana do formatu 8kpTUB,
    # stejne jako v strain_from_nextnano_to_csi.py
    def makeWFkp8state(self, wftype='kp8', sbSpin=0):
        nx = self.nx
        ny = self.ny
        nz = self.nz
        buffers = [self.read_data_from_NN(elem).reshape((nx, ny, nz),
                                                        order='FORTRAN')
                   for elem in self.allDatnames]
        # print buffers[0]
        # ordering in this wf: real amplitudes + imag amplitudes + total density;
        # ordering in nn++ wf: s up, s down, x, y, z up, z, y, z down
        # final ordering: s up, x, y, z up, s down, z, y, z down
        # real first 8, imag second 8
        if (wftype == 'kp8'):
            # conversion to list necessary,
            # would be treated as ndarray otherwise
            ...
            data = list([0*buffers[0], buffers[0], buffers[1], buffers[2],
                        ...
                        0*buffers[0], buffers[9], buffers[10], buffers[11]])
        elif (wftype == 'x1' or wftype == 'x2' or wftype == 'x3'):
            if (sbSpin == 0):
                data = list([buffers[0], 0*buffers[0],
                            0*buffers[0], 0*buffers[0],
                            0*buffers[0], 0*buffers[0],
                            0*buffers[0], 0*buffers[0],
                            0*buffers[0], 0*buffers[0],
                            0*buffers[0], 0*buffers[0],
                ...
                data = list([0*buffers[0], 0*buffers[0],
                            0*buffers[0], 0*buffers[0],
                            buffers[0], 0*buffers[0],
                            0*buffers[0], 0*buffers[0],
                            0*buffers[0], 0*buffers[0],
                            0*buffers[0], 0*buffers[0],
                            0*buffers[0], 0*buffers[0],
    ...
    def get_state_vectors(self, buffer_names):
        norm_squared = 0.0
        sup_values, modulus = self.make_complex_buffer(buffer_names[0],
                                                       buffer_names[8])
        norm_squared += modulus
        sdown_values, modulus = self.make_complex_buffer(buffer_names[1],
                                                         buffer_names[9])
        norm_squared += modulus
        all_x_up, modulus = self.make_complex_buffer(buffer_names[2],
                                                     buffer_names[10])
        norm_squared += modulus
        all_y_up, modulus = self.make_complex_buffer(buffer_names[3],
                                                     buffer_names[11])
        norm_squared += modulus
        all_z_up, modulus = self.make_complex_buffer(buffer_names[4],
                                                     buffer_names[12])
        norm_squared += modulus
        all_x_down, modulus = self.make_complex_buffer(buffer_names[5],
                                                       buffer_names[13])
        norm_squared += modulus
        all_y_down, modulus = self.make_complex_buffer(buffer_names[6],
                                                       buffer_names[14])
        norm_squared += modulus
        all_z_down, modulus = self.make_complex_buffer(buffer_names[7],
                                                       buffer_names[15])
        norm_squared += modulus
    ...
    def make_intermediate_vector(self, buffer_names):
        ...
        sup_values_1, sdown_values_1, all_x_up_1, all_y_up_1, all_z_up_1,\
            all_x_down_1, all_y_down_1, all_z_down_1 =\
            self.get_state_vectors(real_names_1 + imag_names_1)
        sup_values_2, sdown_values_2, all_x_up_2, all_y_up_2, all_z_up_2,\
            all_x_down_2, all_y_down_2, all_z_down_2 =\
            self.get_state_vectors(real_names_2 + imag_names_2)
        # Execute the vector operations leading to the intermediate vector,
        # Row by row, mixing spin and space variables,
        # and initial as final states following
        # Eq. 1.26 from Klenovsky's PhD thesis.
        ...
        intermediate_vector = na.column_stack((all_x_up_2 + all_x_down_2
                                               - all_x_up_1 - all_x_down_1,
                                               all_y_up_2 + all_y_down_2
                                               - all_y_up_1 - all_y_down_1,
                                               all_z_up_2 + all_z_down_2
                                               - all_z_up_1 - all_z_down_1))
    ...
    def adapt_shape(self, in_array):
        real_part = [elem.real.reshape((self.nx, self.ny, self.nz),
                                       order='FORTRAN')
                     for elem in in_array.transpose()]
        imag_part = [elem.imag.reshape((self.nx, self.ny, self.nz),
                                       order='FORTRAN')
                     for elem in in_array.transpose()]
    ...
    def makeTMEhls(self, kp8_1, kp8_2, pol):
        ...
        real = list([pol[0] * 1.0 / na.sqrt(2.0) * (kp8_1[0] * kp8_2[1]
                                                    - kp8_1[1] * kp8_2[0]
                                                    + kp8_1[8] * kp8_2[9]
                                                    - kp8_1[9] * kp8_2[8]
                                                    + kp8_1[4] * kp8_2[5]
                                                    ...
                                                    + kp8_1[12] * kp8_2[13]
                                                    - kp8_1[13] * kp8_2[12])
                    + pol[1] * 1.0/na.sqrt(2.0) *
                    (kp8_1[0] * kp8_2[10] - kp8_1[2] * kp8_2[8]
                     - kp8_1[8] * kp8_2[2] + kp8_1[10] * kp8_2[0]
                    - kp8_1[4] * kp8_2[14] + kp8_1[6] * kp8_2[12]
                    + kp8_1[12] * kp8_2[6] - kp8_1[14] * kp8_2[4]),
                    pol[0] * 1.0/na.sqrt(6.0) * (- kp8_1[0] * kp8_2[1]
                                                 + kp8_1[1] * kp8_2[0]
                                                 - kp8_1[8] * kp8_2[9]
                                                 + kp8_1[9] * kp8_2[8]
                                                 + kp8_1[4] * kp8_2[5]
                                                 ...
                                                 + kp8_1[12] * kp8_2[13]
                                                 - kp8_1[13] * kp8_2[12])
                    + pol[1] * 1.0/na.sqrt(6.0) *
                    (kp8_1[0] * kp8_2[10] - kp8_1[2] * kp8_2[8]
                     - kp8_1[8] * kp8_2[2] + kp8_1[10] * kp8_2[0]
                    + kp8_1[4] * kp8_2[14] - kp8_1[6] * kp8_2[12]
                    - kp8_1[12] * kp8_2[6] + kp8_1[14] * kp8_2[4])
                    - 0.0 * pol[2] * na.sqrt(2.0/3.0)
                    * (kp8_1[0] * kp8_2[3] - kp8_1[3] * kp8_2[0]
                     + kp8_1[8] * kp8_2[11] - kp8_1[11] * kp8_2[8]
                     + kp8_1[4] * kp8_2[7] - kp8_1[7] * kp8_2[4]
                     + kp8_1[12] * kp8_2[15] - kp8_1[15] * kp8_2[12]),
                    pol[0] * 1.0/na.sqrt(3.0) *
                    (- kp8_1[0] * kp8_2[1] + kp8_1[1] * kp8_2[0]
                     - kp8_1[8] * kp8_2[9] + kp8_1[9] * kp8_2[8]
                    + kp8_1[4] * kp8_2[5] - kp8_1[5] * kp8_2[4]
                    + kp8_1[12] * kp8_2[13] - kp8_1[13] * kp8_2[12])
                    + pol[1] * 1.0/na.sqrt(3.0) *
                    (kp8_1[0] * kp8_2[10] - kp8_1[2] * kp8_2[8]
                     - kp8_1[8] * kp8_2[2] + kp8_1[10] * kp8_2[0]
                    + kp8_1[4] * kp8_2[14] - kp8_1[6] * kp8_2[12]
                    - kp8_1[12] * kp8_2[6] + kp8_1[14] * kp8_2[4])
                    + 0.0 * pol[2] * 1.0/na.sqrt(3.0) *
                    (kp8_1[0] * kp8_2[3] - kp8_1[3] * kp8_2[0]
                     + kp8_1[8] * kp8_2[11] - kp8_1[11] * kp8_2[8]
                    + kp8_1[4] * kp8_2[7] - kp8_1[7] * kp8_2[4]
                    + kp8_1[12] * kp8_2[15] - kp8_1[15] * kp8_2[12])])

        imag = list([pol[0] * 1.0/na.sqrt(2.0) *
                     (kp8_1[0] * kp8_2[9] - kp8_1[1] * kp8_2[8]
                      - kp8_1[8] * kp8_2[1] + kp8_1[9] * kp8_2[0]
                     + kp8_1[4] * kp8_2[13] - kp8_1[5] * kp8_2[12]
                     - kp8_1[12] * kp8_2[5] + kp8_1[13] * kp8_2[4])
                     + pol[1] * 1.0/na.sqrt(2.0) *
                     (kp8_1[0] * kp8_2[2] - kp8_1[2] * kp8_2[0]
                      + kp8_1[8] * kp8_2[10] - kp8_1[10] * kp8_2[8]
                     - kp8_1[4] * kp8_2[6] + kp8_1[6] * kp8_2[4]
                     - kp8_1[12] * kp8_2[14] + kp8_1[14] * kp8_2[12]),
                     pol[0] * 1.0/na.sqrt(6.0) *
                     (- kp8_1[0] * kp8_2[9] + kp8_1[1] * kp8_2[8]
                      + kp8_1[8] * kp8_2[1] - kp8_1[9] * kp8_2[0]
                     + kp8_1[4] * kp8_2[13] - kp8_1[5] * kp8_2[12]
                     - kp8_1[12] * kp8_2[5] + kp8_1[13] * kp8_2[4])
                     ...
                     + kp8_1[12] * kp8_2[14] - kp8_1[14] * kp8_2[12])
                     - 0.0 * pol[2] * na.sqrt(2.0/3.0) *
                     (kp8_1[0] * kp8_2[11] - kp8_1[3] * kp8_2[8]
                      - kp8_1[8] * kp8_2[3] + kp8_1[11] * kp8_2[0]
                     + kp8_1[4] * kp8_2[15] - kp8_1[7] * kp8_2[12]
                     - kp8_1[12] * kp8_2[7] + kp8_1[15] * kp8_2[4]),
                     pol[0] * 1.0/na.sqrt(3.0) *
                     (- kp8_1[0] * kp8_2[9] + kp8_1[1] * kp8_2[8]
                      + kp8_1[8] * kp8_2[1] - kp8_1[9] * kp8_2[0]
                     + kp8_1[4] * kp8_2[13] - kp8_1[5] * kp8_2[12]
    ...
    # Transform TME from {s,p_x,p_y,p_z} basis to {s,hh,lh,so} basis
    # UNFINISHED   !!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    def load_alternate_WFs(self, state_1, state_2, wft1, wft2):
        ...
        # Nacteni obou wf:
        # state 1
        # st1 and st2 have be same sizes,
        # otherwise we will get into trouble later,
            ...
            for i in range(len(st.states)):
                if basisType == 'sppp':
                    if local_mode == 'alternate':
                        ...
                        # TODO: check correct account of
                        # direction of scalar product
                        # with conjugation, compared to sign
                        # of imaginary part in
                        # classical implementation.
                        to_append =\
                            self.make_alternate_TME(iter,
                                                    intermediate_vector)
                        to_append = self.adapt_shape(to_append)
                        TME.append(to_append)
                    else:
                        TME.append(self.makeTME(kp8wf[i][0],
                                                kp8wf[i][1], iter))
                elif basisType == 'shls':
                ...
                sumOfME = ((na.sum(P*MEreal)) ** 2
                           + (na.sum(P*MEimag)) ** 2)
                sumOfME_1 = ((na.sum(P * TME[i][0][0])) ** 2
                             + (na.sum(P*TME[i][1][0])) ** 2)
                sumOfME_2 = ((na.sum(P*TME[i][0][1])) ** 2
                             + (na.sum(P*TME[i][1][1])) ** 2)
                sumOfME_3 = ((na.sum(P*TME[i][0][2])) ** 2
                             + (na.sum(P*TME[i][1][2])) ** 2)
                M.append(sumOfME)
                M1.append(sumOfME_1)
                M2.append(sumOfME_2)
                M3.append(sumOfME_3)
